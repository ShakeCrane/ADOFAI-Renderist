...
- **HTTPS 下载（2026-09-24 实施；Unity 实机未验证）**：已按既定路线用 `UnityWebRequest` + `DownloadHandlerFile` 实现固定资产下载，见 §2.6。`FfmpegInstaller` 仍然只接受"已在本地"的归档路径（它**只读取、绝不删除**该文件；临时文件由下载控制器独占拥有）。**L2（FFmpeg 帧流进程）与 L3（MP4 帧事务）均未实现**；`MasterTimeline` / `DeterministicFrameScheduler` / `FrameCaptureDriver` / `EditorExportController` / `EditorExportSession` **未被修改**，未新建第二套 EOF 或第二个 `CommitFrame` authority。
...
**阶段状态**：`0.3.8.0 — FFmpeg Video Export Pipeline` 为已批准目标阶段；当前产品版本仍为 `0.3.7.1`。L1 的发现、固定 manifest、能力探测、本地归档安全安装和信息性 GUI 已实现；审查基线为 `caecfa9`，本地 `main` 当时领先实时核对的 `origin/main=731b186` 四个提交。本节保留 GPT Work 的审查结论；其中"HTTPS 下载未实现"与"probe 非零退出未修复"两条已在 2026-09-24 由 dsh 处理，现状见 §2.6。**L2/L3 仍未实施。**
...
- **已验证层级（2026-09-24 更新）**：独立 net48 回归现为 **67 passed / 0 failed / 1 skipped**（下载管线与 probe 回归并入后，见 §2.6）。跳过项仍是**真实 FFmpeg 多版本能力回归** —— 本机没有 fixture，且**当前环境出站 TLS 被阻断**（DNS 与 TCP 443 可连，TLS 握手失败），无法按 manifest 获取固定资产，因此本轮**没有**取得任何真实 FFmpeg 能力检查结果。Release 主工程构建 0 error / 0 warning；独立发布包校验 **11 checks / 0 failures**，包仍只有三文件。以上不是 Unity Mono HTTPS、GUI 生命周期或真实二进制的实机验收。目标游戏文件当前显示 UnityPlayer `6000.3.10`、`Assembly-CSharp.dll` FileVersion `0.4.3.0`；实际 `UnityEngine.UnityWebRequestModule.dll` 通过只读反射确认 `UnityWebRequest.SendWebRequest/Abort/Dispose`、`result/responseCode/downloadedBytes/redirectLimit/timeout`、`DownloadHandlerFile.removeFileOnAbort` 成员存在，`System.Net.Http.dll` / `System.dll` 也存在。这仍只证明本地程序集条件，不能证明 TLS、回调相位与取消运行语义。
- **已确定的下载实施路线，运行行为待验证**：优先使用 Unity `UnityWebRequest` + `DownloadHandlerFile` 写入独占临时文件。网络请求由 Unity 异步执行；在主线程创建、`SendWebRequest()`、订阅 `UnityWebRequestAsyncOperation.completed`、取消时 `Abort()`、完成后 `Dispose()`；后台 `Task` 只接管已关闭的本地文件哈希与现有 `FfmpegInstaller`，不得访问 Unity 对象。Unity 6 官方文档明确 `completed` 事件在 Update 之后的协程相位调用、同帧注册至少下一帧回调；`DownloadHandlerFile` 直接落盘，`removeFileOnAbort` 应设为 true；`certificateHandler=null` 使用平台默认证书校验。仍须在**实际 ADOFAI Unity Mono** 验证 HTTPS GitHub 固定 tag 下载与重定向、证书失败、断线、进度、取消、禁用 Mod 与退出游戏时的资源收敛；文档与程序集存在性不算通过。此路线不需改变全局 TLS、系统 PATH 或管理员权限。`HttpClient` / `HttpWebRequest` / `WebClient` 暂不作为实现路线；本轮没有它们在目标 Unity Mono 的可靠运行对照。
- **已由 dsh 实施（2026-09-24），运行行为仍待实机验证**：上述要求已落实为 `FfmpegDownloadController`（Unity-free 状态机 / generation 隔离 / 临时文件唯一 owner / 后台校验与安装）与 `UnityFfmpegDownloadDriver`（唯一持有 `UnityWebRequest`，只在主线程 `SendWebRequest` / 读取进度 / `Dispose` / `Abort`）。用户主动点击后才下载 manifest 钉住的 Gyan 9.0.2 ZIP；沿用固定长度 `114768076` 与 SHA-256 `60f467265b1e312373dbcd92200c2618a74850f98d3d078e94296bb3fa2047ba`，响应成功仍须校验**磁盘实际长度**与**重新计算**的内容哈希（`FfmpegFileHash` 已取消元数据缓存）；非 HTTPS 最终地址与 HTTP 非 200 一律 fail-closed，不使用 `latest`。取消/禁用/卸载会**先失效 generation**，再请求 Abort 并同步删除自己那一代的临时文件；迟到通知只能清理自己那一代，绝不启动安装、发布或把已取消会话标记为成功。`OnUpdate` 的 FFmpeg 生命周期泵现在**先于 `Enabled` 判断**执行，且 `OnToggle(false)` 与 `OnUnload` 各自独立收敛，不依赖 GUI 再次打开；OnUnload 仅在签名已核实的范围内接线。仍**未**在实机验证的部分见 §2.6。固定 SHA 与能力报告只是发现时快照；未来 L2 在 `Process.Start` 前必须重新确认所选绝对路径的当前二进制身份并处理校验与启动间的替换竞态，不能把 L1 报告当成永久运行授权。
...
**范围**：在 GPT Work 三处 L1 修正（§2.5，原样保留并回归验证）之上，补齐 `UnityWebRequest` + `DownloadHandlerFile` 下载，使 L1 具备完整功能闭环。**未**接入 L2/L3，**未**修改 `MasterTimeline` / `DeterministicFrameScheduler` / `FrameCaptureDriver` / `EditorExportController` / `EditorExportSession`，产品版本仍为 `0.3.7.1`。
...
1. **Unity Mono 实机未通过（已移交 ChatGPT）**。本轮**没有**取得任何 Unity 实机结果。桌面当时存在**正在运行的第三方 computer-use 控制会话**（`codex-computer-use-swift`，窗口标题 "ChatGPT is using your computer. Esc to cancel"）；继续注入键鼠会与它争用、使操作结果不可信，故按"状态不可靠即停止输入"停止输入，**由用户决定把游戏内验证交给 ChatGPT**。已为该项验证准备好前置状态：新构建已部署且与 `bin\Release` 逐字节一致（DLL SHA-256 `64C9DCEB08CF…`）、游戏进程已启动、UMM 快捷键为 `Alt + F1`。本文件不记录任何未实际执行的实机结论。
2. **HTTPS 下载本身在当前环境无法验证**：本机出站 **TLS 被阻断**（`github.com` / `www.gyan.dev` / `api.nuget.org` 均 DNS 可解析、TCP 443 可连，但 TLS 握手失败），因此下载、GitHub 重定向、进度与"安装后真实能力探测"这四项**既未离线验证、也未实机验证**；Unity Mono 下的 `completed` 相位、`Abort` 收敛与证书校验语义同样未验证。
3. **真实 FFmpeg 多版本能力回归 = SKIP**：本机无 fixture，且无法按 manifest 获取（TLS 阻断）。§2.3 记录的是此前独立进程取证，**不是**本轮新实现跑出来的结果。
4. **L2/L3 风险未消除**：帧流进程（`Process.Start` / 异步流 / `Exited` 时序）、MP4 帧事务、`CommitFrame` 封装关口、主线程回投与视觉确定性（§2.2）均未实现或未验收。GUI 复用现有 IMGUI，但新组件区域的实机渲染、禁用/卸载/退出回调的真实触发顺序也未验证。
5. **许可证义务未定论**：manifest 已展示构建方 / 版本 / 归档来源 / 许可证 / 对应源码入口，实际分发告知义务仍须按发布方式审查。

**结论**：L1 功能代码已实现、离线回归与发布验证通过，但**尚未通过实际验收**；剩余阻塞见上述 1–3（环境性）与 4（后续阶段）。
...
1. **0.3.8.0 视频导出（未完成）**：**组件管理 L1 已实现并通过离线回归**（§2.4），但**下载方案未收敛**（Unity Mono HTTPS 客户端，须由 GPT Work 定案），L2 帧流进程与 L3 MP4 帧事务均**未实施**。Event-Driven A+ 决策见 §2.1。优先验证跨 Unity 帧的 pending、主线程完成投递与长时间 FFmpeg 背压下的原生 Update / visual state 是否漂移，再落实 MP4 接入、Finalizing 和完整故障收敛。静态设计与离线回归**都不是** MP4 运行时验收。
