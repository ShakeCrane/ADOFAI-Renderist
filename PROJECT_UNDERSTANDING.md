...
- **二进制身份变化（重要）**：本轮修正改变了二进制。§2.6 的实机结论对应 GPT Work 的清洁构建 `F8B3553F4622934F5FB46D5BE3AAC547A627031DA207656090CFDE85B9ABB919`；**包含本轮修正的当前源码构建为 `63674902C622D90213F21B12F734D6F497BDEB4820EB9C089CDB55DC168ED4D2`，尚未在游戏内验证**（未重新部署，未重复整套主路径实机下载）。
- **L1 分项状态（不得整体宣布完成）**：**已有实机证据** —— 主下载（固定 HTTPS、GitHub → `release-assets.githubusercontent.com` 重定向、HTTP 200、`Content-Length` = `downloadedBytes` = 实际文件长度 = `114768076`）、内容 SHA-256 校验、安全解压与原子安装、**真实能力探测**（`libx264` / `mp4` / `rawvideo`）、**游戏重启后重新发现为 Ready**。**仍未完整验收** —— 活动下载期间禁用/卸载、旧通知与新请求的真实交错、证书与断线失败路径、游戏退出收敛。
- **剩余最小实机验收项**：① 活动下载进行中禁用 Mod / `OnUnload` / 退出游戏，确认 Abort、临时文件与 `.downloads` 收敛；② 让旧代完成通知与新请求**真实交错**（含重复完成通知），确认真实交错下不发布旧版本、不误删在用归档；③ 证书失败与断线中途失败的 fail-closed 与清理；④ 若要把当前 `63674902…` 构建作为交付基线，需重新部署并重跑一次主路径确认。
...
1. **0.3.8.0 视频导出（未完成）**：组件管理与 UnityWebRequest 下载代码已实现；L1 完成通知丢失缺陷与"重复完成通知误删在用归档"ownership 缺陷均已修复（§2.6 / §2.7）。目标游戏中固定资产下载、SHA 校验、安全安装、真实能力探测、GUI Ready 与重启后重新发现**已有实机证据**；但**含最新修正的构建尚未实机验证**（§2.7），仍需活动期取消/禁用、旧通知与新请求真实交错、证书/断线等边界验收。L2 帧流进程与 L3 MP4 帧事务均**未实施**。Event-Driven A+ 决策见 §2.1；优先验证跨 Unity 帧的 pending、主线程完成投递与长时间 FFmpeg 背压下的原生 Update / visual state 是否漂移，再落实 MP4 接入、Finalizing 和完整故障收敛。L1 Ready **不是** MP4 运行时验收。
