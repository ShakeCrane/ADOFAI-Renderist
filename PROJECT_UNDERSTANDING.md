...
- **二进制身份（2026-09-24 复核，证据来自直接读取部署文件，不由 UI 结果推断）**：游戏 `Mods\ADOFAI.Renderist\` 中**当前部署**的 DLL 为 `F8B3553F4622934F5FB46D5BE3AAC547A627031DA207656090CFDE85B9ABB919`，`ProductVersion = 0.3.7.1+bea143d18e988e1a85897c3206d2a71b3ca02d56`（嵌入 HEAD `bea143d`），文件写入时间 **2026-09-24 13:34:51**。该时间**早于**回调 ownership 修正提交 `ea09bd7`（**2026-09-24 14:11:30**），因此该部署构建**不包含**本轮修正。当前 HEAD（`be29174`）的清洁构建为 **`6EA4835593228ADA527F94EA0350513711796CB74F15472DABA2825FB1233861`**，`ProductVersion = 0.3.7.1+be291741be58870faed162f6dfee9bfc51d298e0`，**尚未部署、尚未实机验证**。两者功能差异**仅为** `ea09bd7` 的 ownership 修正（仅离线回归覆盖）；按要求未重复整套主路径实机下载。（此前记的 `63674902…` 是 HEAD 仍为 `bea143d` 且工作区含未提交修正时的构建，该哈希已作废。）
