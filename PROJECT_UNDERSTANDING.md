...
| 当前开发方向 | **`0.3.8.0 — FFmpeg Video Export Pipeline`：L1 完成并实机验收，L2 独立管线源码收敛及 Mono 定向验证完成（§2.8.3）；§2.2.1 已在单 session 同逻辑帧确认跨帧等待导致视觉漂移**。下一步先由 Web GPT 判定原生视觉补间的时间 ownership / L3 边界，不能直接接入正式 MP4 帧事务。第二闭环仍然**未覆盖** Unity **Linear** 色彩空间与极端资源失败路径（详见 §9.7.3），不得由 Gamma 结果外推。 |
| 下一阶段 | **视觉时间 ownership 决策（§2.2.1）**：已确认 `OnMusicScheduled` 创建的原生 DOTween 在 Renderist 时间冻结时仍推进；应先确定最小范围的等待期控制与复验，再评估 **`L3 — Unity MP4 Frame Transactions`**。Mono L2 探针只证明独立原始帧管线，`0.3.8.0` 的正式 DLL **不具备编辑器 MP4 导出能力**。 |
...
- **终态与隔离（L3 接线待实现）**：FFmpeg stdin 完整写入只说明帧已交付，**不是 MP4 成功**。canonical completion / End Tail 排空后关闭 stdin、异步等待进程退出及 stderr 完成，确认退出码 0 和临时 MP4 有效，再将同卷临时文件发布为最终文件，随后标记 session Completed。取消/失败时废弃当前 generation、关闭管道、终止并回收进程、清理或隔离临时文件；迟到回调不能提交新 session 帧，cleanup residual 未收敛不得重开。后台线程不接触 Unity 对象。Unity 主线程完成通知采用事件驱动调度；当前环境的合成帧 L2 后台完成经 `SynchronizationContext.Post` 回主线程已由 §2.8.3 验证，**正式 L3 帧事务回投仍待验证**，不得用定期轮询 IO 代替。
...
- **未解决 / 待验证**：Unity RGB24 CPU 像素缓冲的长度、行顺序与生命周期；**等待期间原生视觉补间确实漂移**（至少 `OnMusicScheduled` 的 `cosmeticRadius` → 非选中星球 Transform，§2.2.1），其时间 ownership 与其它可见特效尚未收敛；**活跃新 session**取消与旧通知竞态；Gamma / Linear 色彩差异与奇数尺寸 H.264 4:4:4 的目标播放器兼容性。FFmpeg 独立管线在目标 Unity Mono 中已有合成帧正常/取消定向证据（§2.8.3），**仍未接入编辑器帧事务、仍无 Renderist 编辑器 MP4 导出实机验证**。
...
- **当时的阶段判断（已由 §2.2.1 更新）**：Event-Driven A+ 的事件投递、单帧逻辑背压及取消后的闲置态旧通知隔离在本环境具备可行性证据；2026-09-23 时尚不能判定视觉差异是否由等待造成。§2.2.1 的同 session 同帧证据已确认至少一种原生视觉漂移，故不再要求重复独立 session 的无延迟 / 延迟 PNG 对照。此次历史探针已清理，不把其等待门控当作稳定产品功能，也不更改 `0.3.7.1` 历史基线。Event-Driven A+ 的正式 MP4 帧事务接入仍未获准。

### 2.2.1 单 session 同逻辑帧视觉因果验证（2026-09-26；探针已清理）

在当前 `0.3.8.0` / Unity `6000.3.10f1` Mono、Gamma / Direct3D11、3072×1920、30 FPS、scale=1 下，临时 Log-only 探针于**同一个 session 的 pre-entry 逻辑帧 0**完成 Prepare / 首次 EOF 后截取 A，再保持该帧 pending，暂停 Renderist 的下一帧 Prepare、autoplay、Commit 和时间推进，但让 Unity 原生 Update / render loop 继续；下一 EOF 截取 B，长等待后截取 C。三次均走相同 Source RT → RGB24 ReadPixels / Apply 路径，不经过 FFmpeg。探针完成后已删除诊断源码、GUI、instrumentation 与图片；正式 L2 源码无该等待行为。此次只测试 frame 0；因为这里已确认漂移，未继续扩展到 B−1 / G 或 gameplay。

- 实机 session `editor_20260926_123330` 的 A/B/C 均为 `outputFrameIndex=0`、`frameTransactionRequestCount=1`、主线程 ID 1、`Countdown`、forced songposition `-0.17000000178813934`（实际 getter 均显示 `-0.170000001788139`）、`timeScale=1`；没有第二次 session 的初始状态混淆。Unity `Time.frameCount=1275 → 1276 → 3354`，A→C 约 6.26 秒、2079 个 Unity frames。相同尺寸 RGB24 像素差异：A→B **122,525 / 5,898,240 = 2.0773%**，B→C **178,460 = 3.0256%**，A→C **182,955 = 3.1019%**；最大通道差分别 230 / 255 / 255。三次诊断运行均在 frame 0 重现漂移，但这些百分比只代表该谱面、该状态与该配置。
- 同帧状态与像素对应：选中 `PlanetRed` 的 angle `-11.885693`、Transform `(0,0,0)`，三台 Camera 未见明显位置/旋转变化（相机 Transform 日志按两位小数格式化，不能排除微小变化），控制状态不变；其 `cosmeticRadius` 却为 **0.693333 → 0.833333 → 1.5**。未选中 `PlanetBlue` 的 Transform 为 **(0.43632862, 0.5388214) → (0.524433434, 0.64762187) → (0.9439801, 1.16571927)**，呈同方向径向移动；实际图片中蓝色星球随之移动。`PlanetBlue` 自身的 angle、`cosmeticRadius`、`perc/easedPerc` 在所测点未变。背景/发光区域也有像素差，尚不能把全部差异都归因于该星球路径；Trail、Particle、Shader 等未逐一排除。
- 当前游戏 `Assembly-CSharp.dll` FileVersion `0.4.3.0` 的 IL 核对：`scrController.OnMusicScheduled` 构造 `DOTween.To`，setter `scrController+<>c__DisplayClass171_0.<OnMusicScheduled>b__1` 写选中星球的 `cosmeticRadius`，目标为 `tileSize × currFloor.radiusScale`，duration 为 `popuptime`；`scrPlanet.Update_RefreshAngles` 使用该半径计算并 `Transform.set_position` 到另一星球。实机半径与位置序列符合这条链。**已确认至少这条原生补间在 Renderist logical frame 冻结时继续推进，并造成可见像素漂移**；并未证明其它特效无漂移，也未验证正式 L3 IO 等待。
- **边界决定**：不能把“逻辑时间/选中星球 angle 未变”当作画面冻结证据，也不能直接把 L2 Mono IO PASS 外推到编辑器 MP4 的视觉确定性。正式控制此原生 DOTween 可能需要限定范围的 tween 时间 ownership / Hook / render timing 决策，超出本轮调查边界；由 Web GPT 先决定 L2→L3 范围，之后只针对实际机制做最小修复与同帧复验。未实施 TEMP 修复，未新增正式 Harmony Patch、未修改 `CommitFrame` 或既有 PNG / Log-only 行为。
...
### 2.8 L2 — FFmpeg Video Process Pipeline（源码阶段已收敛；未接入 L3）

**基线**：在 `7a67546a62bf02e7079ebf77cbecfcd282d8b0e2` 之上实现，产品版本**保持 `0.3.8.0`**（用户要求不递增第四位）。Phase 文案在实现轮**保持 L1**；在随后的 **L2 源码收敛轮**按用户指示更新为 `Phase 3.8.0 FFmpeg Video Export Pipeline — L2 Video Process Pipeline`，同步点见 §12.3。新增源码（全部 Unity-free / UMM-free / Harmony-free，因此同一份源码直接编入 net48 回归工程）：
...
- **冻结配置**（GPT Work 复审修复）：构造时深复制全部标量（宽高 / FPS / CRF / preset / 像素格式）与路径、编码器身份、期望编解码器名；之后调用方修改 `FfmpegVideoPipelineOptions` / `FfmpegVideoSettings` 不再影响本会话 —— 回归同时覆盖**构造之后**与 **Start 之后**再次修改原对象两种时机。
- **初始化逐项登记 ownership**（最终复审 P2 修复，见 §2.8.2）：`process` / `stdin` / exit-wait task / stdout pump / stderr pump **每一个创建或启动成功之后立即登记**，不存在"已启动但尚未登记"的窗口；因此即使 stderr 初始化失败，已经运行的 stdout pump 仍归本会话所有，`CleanupTask` 会等待它收敛，进程也不会在后台读取任务仍持有管道时被 `Dispose`。
...
**本轮实际验证结果**（L2 实现轮 → 首次复审六项修复轮 → 最终复审一项 P2 修复轮；全部为独立 net48 进程，数字为本轮实际执行所得）：

- 离线（无 fixture）`tests/run-ffmpeg-tests.ps1` → **129 passed / 0 failed / 12 skipped**（12 个真实 fixture 用例如实 SKIP）。
- 以真实 Gyan 9.0.2 essentials（与 manifest 同哈希）为 fixture → **141 passed / 0 failed / 0 skipped**。
- 覆盖：命令与冻结配置、Windows 参数引用（含空格 / 中文 / `&` / 括号 / emoji 的真实子进程往返）、身份冻结与失效检测、核验协议正负对照、单帧在途与 busy、部分写入污染、中途断管、空输入、非零退出、核验失败、启动失败、输出目录缺失、临时文件已存在、正式目标文件保护（启动前与 session 中途出现两种情形）、Finalizing 中取消、发布后取消、终态竞争、Dispose 非阻塞、连续启停无残留、迟到写入结果不增加计数、清理失败 residual 报告、真实编码的取消与进程回收；修复轮另加 §2.8.1 的 12 项与 §2.8.2 的 1 项定向回归。
...
- **未接入 Unity 导出流程**：L2 只提供"接受外部完整 RGB24 帧"的独立管线，虽有 §2.8.3 的目标游戏定向探针，**`0.3.8.0` 仍然不是编辑器 MP4 导出可用的版本**；没有 MP4 导出 UI、没有音频、没有 replay、没有新增 Harmony Patch。
- **本节的实现轮测试是独立 net48 进程结果**，不能单独当作 Unity Mono 实机证据；后续目标游戏定向结果见 §2.8.3。L3 帧事务与编辑器 MP4 的 Finalizing 关口仍未验证。
- **本节实现轮当时**未在游戏内部署，也未改动当时 `Mods` 内的 L1 构建；后续诊断部署与正式 L2 DLL 恢复见 §2.8.3 / §12.3。
- **工具环境勘误（2026-09-26 复测）**：§2.8 实现轮的 agent 环境曾观察到托管 FFmpeg 无法写输出；该结论**不适用于当前环境**。当前直接使用托管的同一 Gyan 9.0.2 `ffmpeg.exe`，输出放在短路径 `LocalLow\L2tmp` 时真实 fixture 回归 **141 / 0 / 0**；默认 `%TEMP%` 下 FFmpeg 即使输出文件未预创建仍报 `Permission denied`，而过长的游戏 LocalLow 临时路径又使部分 L1 staging marker 写入触及路径长度失败。测试临时目录位置会改变本机结果，不能把这些差异归因为 L2 代码或正式游戏运行时。
...
**保留与未变**：单帧在途与 `busy` 语义、终态仲裁与发布语义、`-y` + 显式 `-f mp4`、核验协议与负对照、L1 身份契约与 TOCTOU 说明、无固定写入超时。**在本次复审轮当时仍未验证**：Unity Mono 实机（进程/异步 IO/`Exited` 语义、主线程回投）、L3 帧事务与 Finalizing 关口；本轮**未部署**游戏。后续 Mono 定向结果见 §2.8.3；L3 仍未实施。

**剩余风险（不编造保证）**：ownership 句柄释放之后到 `Move`/`Delete` 完成之间存在一个极短窗口，同一用户的其他进程理论上可以在该窗口内替换文件；窗口内只有两个相邻 BCL 调用，且路径是会话目录下 16 位随机 token 的临时名，正常流程没有外来参与者。若需要零窗口，需要按文件 ID 校验身份（P/Invoke `GetFileInformationByHandle`）或内核级独占语义 —— 本轮**未**引入该依赖，仍然保留为**已知、非阻断的 threat-model 风险**。

#### 2.8.2 最终复审补充的 P2：初始化资源必须逐项登记 ownership（已修复）

**GPT Work 最终复审结论**：初始化顺序为"取得 stdin → 建立退出任务 → 启动 stdout pump → 启动 stderr pump → 最后一次性批量登记字段"。如果 stderr 取得或包装抛错，stdout pump 已经运行，但 `_stdoutPump` 仍为空，`Teardown` 无法等待它，进程可能在后台读取任务仍持有管道时被提前 `Dispose`。

**实测确认**：源码中该批量登记确实存在（同一 `lock` 内一次写入 `_stdin` / `_exitTask` / `_stdoutPump` / `_stderrPump`），因此"已启动但未登记"的窗口真实存在。

**修复**：改为**逐项登记** —— `process` 在 `Process.Start` 成功后立即登记（沿用上一轮），随后 `_stdin`、`_exitTask`、`_stdoutPump`、`_stderrPump` 各自在创建/启动成功的下一步之前登记；只有全部登记成功后才把状态推进为 `Running`。失败路径不变：`process-init-failed` + 收敛任务回收（已登记的进程、stdin、退出任务与可能已启动的排空泵全部由 `Teardown` 等待并释放）。

**新增确定性回归（1 项）**：`hardening: a failed stderr initialization still owns and waits for the running stdout pump`
- 场景：进程真实启动；stdout 经 `StreamWrapper` 包装为 **可控屏障流**（`BarrierReadStream`，进入读取时置位、等待释放）；stderr 包装直接抛 `IOException`。
- 在屏障**未释放**时断言：`CleanupTask` 未完成（`Wait(1500)` 为 false）、状态为 `Failed`、编码进程确实启动过。这是漏洞判据 —— 旧行为下 `CleanupTask` 会立即完成。
- 释放屏障后断言：`CleanupTask` 完成且终态 `Failed` / `process-init-failed`；`ProcessReaped` + `ProcessDisposed` + `TempOwnershipReleased` + `TempFileRemoved` 均为真；无 residual ownership；无临时文件、无已发布产物；核验进程从未启动；子进程计数不增加。

**"复现旧缺陷"验证**：临时把初始化改回**批量登记**后重跑整套件 → **140 passed / 1 failed**，唯一失败的正是上述新测试（`CleanupTask must not complete while an owned stdout pump is still running`）；还原并强制重建后 **141 / 0 / 0**。

**保留不变**：正常成功路径的初始化行为、编码命令、核验协议、终态仲裁与发布语义、L1 身份契约；测试用故障注入缝（`WriteCommitBarrier` / `StreamWrapper`）继续保留 —— 它们是**生产恒为 null** 的可控同步手段，用于让"已启动但未提交/未登记"这类时序漏洞可以被确定性复现，而不是靠 `Thread.Sleep` 碰撞窗口。

#### 2.8.3 Unity Mono L2 定向验证（2026-09-26；独立原始帧管线）

**证据与构建身份**：基线 `main = origin/main = 9a7daffa95c4e3f233f8538c8dc79b97ef31be9e`，产品版本 `0.3.8.0`。在 Unity `6000.3.10f1` / Mono、UMM `0.33.0` 的目标游戏内，用户显式触发临时 UMM 诊断入口；部署 DLL SHA256 `0D90CA6A990EA814854C3C5D3BC37990A486ACF12DF2751C1C2D2C84DAF173C6`，本地构建与 Mods 内文件哈希一致。探针调用**正式** `FfmpegVideoPipeline` / `FfmpegVideoVerifier`，从 L1 `Ready` 报告取得冻结身份，输入合成 RGB24 帧；为确定性制造取消窗口，临时把正式 Start 中的 stdin 也接到既有 `StreamWrapper` 测试缝，包装流先向真实 Mono 管道写 4096 字节，再保持该帧 WriteAsync 在途。stdout/stderr 的包装只记录 EOF。**此诊断 DLL 与 §12.3 的正式 L2 DLL 不是同一二进制**；临时缝已全部回退，不应把探针结果宣称为未改动正式 DLL 的单独重启验收。证据是目标游戏 `Player.log` 的两代 `[TEMP Mono L2]` 记录（normal / cancel）；探针 MP4 及代目录均在运行后删除。

- **正常场景 PASS**：8 次完整顺序写入；`Running → Finalizing → Completed`，编码进程 PID 有记录，stdout/stderr 均观察到 EOF，`Exited` 事件到达，独立 verifier 为 `Verified`；实际可解码帧数 = 容器包数 = 已交付帧数 = **8**，时间基 `1/30`，正式目标经同目录发布且非空。终态记录 `exit=0`、`reaped=True`、`disposed=True`、`tempRemoved=True`、`handleReleased=True`、`residual=False`、`cleanupTaskCompleted=True`。文件发布后探针删除其私有输出目录。
- **写入中取消 PASS**：编码进程已启动，首帧部分数据已进入真实 Mono stdin，写入等待可控门控时 `Cancel` 返回 accepted，状态立即为 `Cancelled`；随后写入返回 `cancelled`，已交付帧数仍为 **0**，无正式发布。stdout/stderr EOF、`Exited` 均到达；终态 `reaped=True`、`disposed=True`、`tempRemoved=True`、`handleReleased=True`、`residual=False`、`cleanupTaskCompleted=True`；输出代目录已删除。此测试覆盖一次确定性写入中取消，**不**覆盖所有取消竞态或正式 L3 session 隔离。
- **目标 Mono API 语义**：探针观察到 `Process.Start`、异步 stdin 写入、异步双流排空、`Exited`、Finish/Finalizing、核验与发布在正常场景可收敛；另起的短 FFmpeg `-version` 进程通过 `WaitForExit` 返回 exit 0，正式管线两代均报告已回收/已 Dispose。当前两次事件日志中 EOF 先于 `Exited` 记录，**不构成保证顺序**；仍必须独立等待退出与双流 EOF。`FileMode.CreateNew` 对已占有路径拒绝二次创建；保持 `FileShare.ReadWrite` 且不共享 Delete 的句柄时，Delete / Replace 被拒绝，释放后 Move / Delete 成功。既有句柄释放到 Move/Delete 的极短竞态仍见 §2.8.1。
- **主线程回投与持续运行**：`SynchronizationContext.Current` 为 `UnityEngine.UnitySynchronizationContext`，main thread ID = 1。两代后台完成都经 `Post` 在 thread 1 回调（正常场景 frame 13914→14048、期间 135 次 Update；取消场景 frame 14244→14366、期间 123 次 Update），说明该探针运行期间主线程未被同步阻塞，Mod 更新循环继续执行。**限制**：这次游戏由直接启动方式运行，日志另有 `SteamApi_Init failed with NoSteamClient` 及随后游戏 `NullReferenceException`；两项错误发生在探针前，不能归因 L2，也不能据此宣称整个游戏或既有 PNG / Log-only 路径健康。正式 L2 DLL 恢复部署后未重新启动游戏。
- **清理与下一步**：临时 `MonoL2Probe.cs`、UMM GUI/Update 接线、stdin 包装行均已删除/回退；正式生产源码与 `9a7daff` 一致。诊断 UMM 缓存已删除，Mods 已恢复 §12.3 的正式 DLL `3FBB993F…`；正式 ZIP 未变。清理后 Release Rebuild 0 error（`NU1900` 网络警告），真实托管 Gyan 9.0.2 fixture 在可写短 LocalLow 临时目录回归 **141 passed / 0 failed / 0 skipped**。**Mono L2 独立管线定向验证通过；编辑器 MP4 仍不可用。**进入 L3 规划前仍需 §2.2 的视觉确定性专项验证，尤其是控制初始状态后判断跨帧等待是否导致画面漂移。
...
1. **0.3.8.0 视频导出（L1 完成并实机验收；L2 独立管线源码收敛及 Mono 定向验证通过；L3 未开始）**：当前 Phase 为 `Phase 3.8.0 FFmpeg Video Export Pipeline — L2 Video Process Pipeline`。L1 完成通知与归档 ownership 缺陷已修复；主路径实机证据见 §2.6 / §2.7，仍未验收活动下载期间禁用/卸载、异常网络及真实回调竞态。L2 独立 net48 真实 Gyan fixture 回归 **141/0/0**；目标 Unity Mono 的正常 8 帧编码/核验/发布及写入中取消/回收两项定向探针均 PASS（§2.8.3），证据绑定临时 DLL `0D90CA6A…`。正式 L2 DLL `3FBB993F…` 已恢复部署，但尚未单独以原样 DLL 重启运行。**L3（编辑器 MP4 帧事务 / Finalizing 关口）未实施**，`0.3.8.0` 仍不能从编辑器导出 MP4。§2.2.1 单 session 同帧实验已**确认**跨帧等待期间 `OnMusicScheduled` 原生 DOTween 使星球位置与输出像素漂移；下一步由 Web GPT 决定视觉时间 ownership 与 L3 边界，不能用 L2 Mono IO PASS 替代该决策，也不再重复独立 session PNG 对照。
...
- **版本收敛：`0.3.7.1` → `0.3.8.0`（2026-09-24 执行，用户裁定）**：**此前建议采用 `0.3.7.2` 的方案已由用户明确作废**（正确目标为 `0.3.8.0`；**不得使用 `0.3.7.2`**）。本轮直接开启 `0.3.8.0` 版本线，并采用 Phase `Phase 3.8.0 FFmpeg Video Export Pipeline — L1 Component Management`，使版本号与 Phase **同时**表达「当前开发阶段」与「已完成的功能」：`0.3.8.0` + `Phase 3.8.0` 表示已进入 FFmpeg 视频导出管线阶段，`— L1 Component Management` 子标题表示当时**仅**完成 L1。**该时点 L2 帧流进程与 L3 MP4 帧事务均未实施（L2 现已实现，见 §2.8；L3 仍未实施），因此不得因进入 `0.3.8.0` 而宣称 MP4 导出可用。** 最终源码提交 `4d35801` 为**承载 `0.3.8.0` 版本变更的提交**。`0.3.8.0` 正式构建（`AE2326D3…` / `+4d35801`）已部署并通过用户最小实机验收（§2.7）；此前 `1D70BE45…`（`+02c7fb8`）为收敛前的 L1 源码证据，保留于 §2.7。**后续补充**：L2 源码收敛轮（§2.8.2 / §12.3）在**不改版本号**的前提下把 Phase 文案更新为 `… — L2 Video Process Pipeline`；版本号与 L1 构建身份均未改变。
...
### 12.2 L1 发布身份（`0.3.8.0` — L1 Component Management；已通过 L1 主路径实机验收）

> 本节记录已通过 **L1 主路径**实机验收的正式 `0.3.8.0` 构建（其 Phase 文案是**该构建当时**携带的 L1 文案）；后续 §2.8.3 的 Mono L2 证据绑定另一份**临时诊断** DLL，不能混为同一构建。**代码中当前的 Phase 文案自 L2 源码收敛轮起为 `Phase 3.8.0 FFmpeg Video Export Pipeline — L2 Video Process Pipeline`**（§12.3）。
